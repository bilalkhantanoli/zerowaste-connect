...
export default function RecipientDashboard() {
  ...
  return (
    <DashboardLayout navItems={navItems} title="Recipient Dashboard">
      <div className="mb-8">
        <h2 className="mb-2 text-2xl font-bold">Welcome, {user?.name ?? 'Recipient'}!</h2>
        <p className="text-muted-foreground">
          <span className="font-semibold text-primary">{donations.length} donations</span> available
          near you today.
        </p>
      </div>
              ...
              <Card key={donation.id} variant="interactive" className="overflow-hidden">
                ...
                <CardContent className="p-4">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      if (!user) return;
                      await requestDonation(donation.id, user.id);
